...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled showLicense:(BOOL)showLicenseButton {
  ...
  if( self ) {
    ...
    if (self.cancelEnabled) {
      UIButton *butt = [UIButton buttonWithType:UIButtonTypeCustom];//[UIButton buttonWithType:UIButtonTypeRoundedRect];
      self.cancelButton = butt;
      if ([self.cancelButtonTitle length] > 0 ) {
        //[cancelButton setTitle:self.cancelButtonTitle forState:UIControlStateNormal];
        [cancelButton setImage:[UIImage imageNamed:@"responseno.png"] forState:UIControlStateNormal];
      } else {
...
- (void)layoutSubviews {
  ...
  if (cancelButton) {
    if (oneDMode) {
      ...
      CGSize theSize = CGSizeMake(54, 54);
